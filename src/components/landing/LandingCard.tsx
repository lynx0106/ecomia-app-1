...
import { deleteLandingFromForm, updateLandingFromForm } from '@/app/actions/landing-pages';
...
export default function LandingCard({ landing, stores, readOnly }: LandingCardProps) {
  ...
  useEffect(() => {
    ...
    if (deleteState?.ok && !lastDeleteRef.current.ok) {
      toast({ title: 'Landing eliminada', tone: 'success' });
  ...
  return (
    <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-5 space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500">
        ...
        {landing.store_id && <span>Store: {landing.store_id}</span>}
        <span>Creada: {new Date(landing.created_at).toLocaleDateString()}</span>
        <PillLink href={`/landing/${landing.id}`} variant="indigo" size="xs">
          Ver detalle
        </PillLink>
        {landing.slug && (
          <PillLink href={`/l/${landing.slug}`} variant="neutral" size="xs">
            Vista publica
          </PillLink>
        )}
      </div>
